(* Copied from iterative_methods/floatlib.v, should really unify these libraries somehow *)
...
Definition faster_matrix_transpose {t} (A: list (list t)) : list (list t) :=
 match A with row::_ => faster_transpose' row A | _ => nil end.

Definition matrix_transpose {t} (A: matrix t) : matrix t :=
 match A with nil => nil
 | row::_ => matrix_by_index (length row) (matrix_rows_nat A) 
               (fun i j => matrix_index A j i)
 end.

Lemma faster_matrix_transpose_correct:
  forall {t} cols (A: matrix t),
  matrix_cols A cols ->  
  faster_matrix_transpose A = matrix_transpose A.
Proof.
 intros.
 destruct A. reflexivity.
 unfold faster_matrix_transpose, matrix_transpose.
 inversion H. subst. clear H3.
Abort.  (* likely provable from here. *)
